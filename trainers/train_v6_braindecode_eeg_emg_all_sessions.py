"""Train and test one-vs-rest EEG+EMG fusion models across all days for one patient."""
...
def add_patient_day_args(parser):
    ...
    parser.add_argument("--emg-file", default="X_emg.npy")
...
def apply_patient_day_paths(args, cli_args):
    ...
    args.emg_file = cli_args.emg_file
...
class EEGDataset(Dataset):
    def __init__(self, X, X_emg, y, indices):
        self.X, self.X_emg, self.y, self.indices = X, X_emg, y, indices
...
class EEGEMGFusionModel(nn.Module):
    ...
    def forward(self, X):
        X_eeg, X_emg = X
        eeg_logits = self.eeg_model(X_eeg)
        eeg_feature = getattr(self.eeg_model, "_eeg_feature", None)
        if eeg_feature is None:
            eeg_feature = eeg_logits.flatten(1)
        emg_feature = self.emg_mlp(X_emg)
        self._emg_feature = emg_feature
...
def initialize_model_and_log_shapes(model, sample_batch, device, log_path, should_log):
    ...
    if should_log:
        X_eeg, X_emg = sample_batch
        shape_lines.append(f"EEG input: {tuple(X_eeg.shape)}")
        shape_lines.append(f"EMG input: {tuple(X_emg.shape)}")
        ...
        for name, module in model.eeg_model.named_modules():
            if name and not any(module.children()):
                hooks.append(module.register_forward_hook(make_hook(name)))

    model.eval()
    with torch.no_grad():
        sample_batch = tuple(item.to(device) for item in sample_batch)
    ...
    if should_log:
        if getattr(model.eeg_model, "_eeg_feature", None) is not None:
            shape_lines.append(f"EEG feature vector: {tuple(model.eeg_model._eeg_feature.shape)}")
        if getattr(model, "_emg_feature", None) is not None:
            shape_lines.append(f"EMG feature vector: {tuple(model._emg_feature.shape)}")
...
def load_all_patient_days(args):
    ...
    day_dirs = [
        ...
        and (day_dir / args.emg_file).exists()
        and (day_dir / "y.npy").exists()
    ]
    if not day_dirs:
        raise FileNotFoundError(
            f"No day folders with {args.input_file}, {args.emg_file}, and y.npy "
            f"found under {patient_dir}"
        )

    X_parts, X_emg_parts, y_parts, day_id_parts, day_names = [], [], [], [], []
    expected_eeg_shape = None
    expected_emg_shape = None
    for day_id, day_dir in enumerate(day_dirs):
        X_day = np.load(day_dir / args.input_file).astype(np.float32)
        X_emg_day = np.load(day_dir / args.emg_file).astype(np.float32)
        y_day = np.load(day_dir / "y.npy").astype(np.int64)
        if len(y_day) != X_day.shape[0]:
            raise ValueError(
                f"{day_dir.name}: EEG and y.npy trial counts do not match: "
                f"{X_day.shape[0]} != {len(y_day)}"
            )
        if X_emg_day.shape[0] != X_day.shape[0]:
            raise ValueError(
                f"{day_dir.name}: EEG and EMG trial counts do not match: "
                f"{X_day.shape[0]} != {X_emg_day.shape[0]}"
            )
        if expected_eeg_shape is None:
            expected_eeg_shape = X_day.shape[1:]
            expected_emg_shape = X_emg_day.shape[1:]
        elif X_day.shape[1:] != expected_eeg_shape:
            raise ValueError(
                f"{day_dir.name}: expected EEG shape (*, {expected_eeg_shape}), "
                f"got {X_day.shape}"
            )
        elif X_emg_day.shape[1:] != expected_emg_shape:
            raise ValueError(
                f"{day_dir.name}: expected EMG shape (*, {expected_emg_shape}), "
                f"got {X_emg_day.shape}"
            )
        X_parts.append(X_day)
        X_emg_parts.append(X_emg_day)
    ...
    return (
        ...
        np.concatenate(X_emg_parts, axis=0),
...
def load_config(config_path):
    ...
    config.setdefault("emg_feature_dim", 64)
...
def train_task(
    ...
    if args.weighted_sampler:
        ...
        train_loader = DataLoader(
            EEGDataset(X, X_emg, y, train_idx),
    ...
    val_loader = DataLoader(
        EEGDataset(X, X_emg, y, val_idx),
        batch_size=args.batch_size,
        shuffle=False,
    )
    test_loader = DataLoader(
        EEGDataset(X, X_emg, y, test_idx),
        batch_size=args.batch_size,
        shuffle=False,
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for X_batch, y_batch in train_loader:
            X_batch = tuple(item.to(device) for item in X_batch)
...
def main():
    ...
    write_log(log_path, f"  emg_file={args.emg_file}")
    ...
    write_log(log_path, f"  emg_feature_dim={args.emg_feature_dim}")
    ...
    X, X_emg, original_y, day_ids, day_names = load_all_patient_days(args)
    if X.shape[-1] < 32:
        raise ValueError("Input time dimension is too short for EEGNet pooling.")

    write_log(log_path, f"Loaded days: {', '.join(day_names)}")
    write_log(log_path, f"Device: {device} | X: {X.shape} | X_emg: {X_emg.shape} | run: {display_path(run_dir)}")
    ...
    run_config = {
        ...
        "emg_input_shape": list(X_emg.shape),
        "days": day_names,
    ...
    if not args.run_5fold_cross_validation:
        ...
        write_log(log_path, "Normalizing X_emg using training-set mean and std...")
        emg_train_mean = X_emg[train_idx].mean(axis=(0, 2))[:, None].astype(np.float32)
        emg_train_std = X_emg[train_idx].std(axis=(0, 2))[:, None].astype(np.float32)
        emg_train_std = np.maximum(emg_train_std, 1e-6)
        X_emg = ((X_emg - emg_train_mean) / emg_train_std).astype(np.float32)
    ...
    if args.run_5fold_cross_validation:
        ...
        with cv_csv_path.open("w", newline="", encoding="utf-8") as csv_file:
            ...
            for fold, (remaining_idx, fold_test_idx) in enumerate(
                ...
                fold_emg_mean = cv_source_emg[fold_train_idx].mean(axis=(0, 2))[:, None].astype(np.float32)
                fold_emg_std = cv_source_emg[fold_train_idx].std(axis=(0, 2))[:, None].astype(np.float32)
                fold_emg_std = np.maximum(fold_emg_std, 1e-6)
                fold_X_emg = ((cv_source_emg - fold_emg_mean) / fold_emg_std).astype(np.float32)
