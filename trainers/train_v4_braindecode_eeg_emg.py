...
def main():
    ...
    write_log(log_path, f"Loading {args.emg_file}...")
    X_emg = np.load(args.data_dir / args.emg_file, mmap_mode="r")
    if X_emg.shape[0] != X.shape[0]:
        raise ValueError(f"EEG and EMG trial counts do not match: {X.shape[0]} != {X_emg.shape[0]}")
    ...
    if args.run_5fold_cross_validation:
        ...
        cv_source_emg = np.load(args.data_dir / args.emg_file, mmap_mode="r")
