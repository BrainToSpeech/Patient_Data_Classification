...
def make_lda_features(X, train_mean, train_std, n_time_bins):
    X = (X - train_mean) / train_std
    if n_time_bins <= 0:
        raise ValueError("--lda-time-bins must be positive.")
    if n_time_bins > X.shape[-1]:
        raise ValueError(
            f"--lda-time-bins ({n_time_bins}) cannot exceed selected samples ({X.shape[-1]})."
        )

    chunks = np.array_split(np.arange(X.shape[-1]), n_time_bins)
    X = np.stack([X[:, :, chunk].mean(axis=2) for chunk in chunks], axis=2)
    return X.astype(np.float32).reshape(X.shape[0], -1)
...
def main():
    ...
    parser.add_argument("--lda-time-bins", type=int, default=16)
    ...
    print(f"Normalizing and binning epochs into {args.lda_time_bins} time bins for LDA...")
    X = make_lda_features(X, train_mean, train_std, args.lda_time_bins)
    print(f"LDA feature shape: {X.shape}")
