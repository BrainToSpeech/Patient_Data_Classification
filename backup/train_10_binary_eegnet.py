...
class EEGNet(nn.Module):
    def __init__(self, n_channels, sfreq):
        ...
        temporal_kernel = int(round(0.5 * sfreq)) | 1
        separable_kernel = int(round(0.125 * sfreq)) | 1

        self.features = nn.Sequential(
            nn.Conv2d(1, 8, (1, temporal_kernel), padding="same", bias=False),
            nn.BatchNorm2d(8),
            ...
            nn.AvgPool2d((1, 4)),
            nn.Dropout(0.5),
            nn.Conv2d(16, 16, (1, separable_kernel), padding="same", groups=16, bias=False),
        ...
        self.classifier = nn.Linear(16 * 4, 2)
...
def main():
    ...
    for label_name in LABEL_NAMES:
        ...
        torch.manual_seed(args.seed)
        model = EEGNet(n_channels, sfreq).to(device)
